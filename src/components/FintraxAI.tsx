...
function ActionBadge({ action }: { action: AgentAction }) {
  ...
  const labels: Record<string, { icon: React.ReactNode; text: string; color: string; bg: string }> = {
  ...
  if (!meta) return null;
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 8, padding: '3px 10px', background: meta.bg, color: meta.color, borderRadius: 99, fontSize: 11, fontWeight: 600, border: `1px solid ${meta.color}22` }}>
      {meta.icon} {meta.text}
    </div>
  );
}
...
export default function FintraxAI({ mode = 'floating', initialOpen = false }: FintraxAIProps) {
  ...
  const clearHistory = useCallback(async () => {
    if (!user) return;
    const name = profile?.displayName || profile?.username || 'there';
    const welcome: ChatMessage = { role: 'assistant', content: `Hey ${name}! Fresh start — what can I help you with?`, timestamp: new Date(), action: { type: 'NONE' } };
    setMessages([welcome]);
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev.filter(m => !m.isTyping), { role: 'assistant', content: 'Sorry, hit a snag. Try again in a moment.', timestamp: new Date(), action: { type: 'NONE' } }]);
    }
    setLoading(false);
  }, [input, loading, messages, expenses, profile, symbol, executeAction, user]);
  ...
  const chatContent = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ padding: '8px 14px', borderBottom: '1px solid #f5f5f4' }}>
          {anomalies.map((a, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '8px 10px', marginBottom: i < anomalies.length - 1 ? 4 : 0, background: a.severity === 'warning' ? '#fffbeb' : '#eff6ff', border: `1px solid ${a.severity === 'warning' ? '#fcd34d' : '#bfdbfe'}`, borderRadius: 8, fontSize: 12 }}>
              {a.severity === 'warning' ? <AlertTriangle size={14} color="#d97706" style={{ flexShrink: 0, marginTop: 1 }} /> : <Lightbulb size={14} color="#3b82f6" style={{ flexShrink: 0, marginTop: 1 }} />}
              <div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row', alignItems: 'flex-start', gap: 8, animation: 'msgIn 0.2s ease' }}>
            {msg.role === 'assistant' && (
              <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}>
      ...
      {messages.length === 1 && (
        <div style={{ padding: '0 14px 10px', display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {QUICK_PROMPTS.map((p, i) => (
            <button key={i} onClick={() => sendMessage(p)} style={{ padding: '5px 10px', background: 'white', border: '1px solid #e7e5e0', borderRadius: 99, fontSize: 11, fontWeight: 500, color: '#57534e', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
      ...
      {/* Input */}
      <div style={{ borderTop: '1px solid #f5f5f4', padding: '12px 14px', display: 'flex', gap: 8, alignItems: 'flex-end', background: '#fff' }}>
        <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
          placeholder="Ask anything or give a command…" rows={1} disabled={loading}
          style={{ flex: 1, resize: 'none', border: '1px solid #e7e5e0', borderRadius: 12, padding: '9px 12px', fontSize: 13, fontFamily: "'DM Sans',sans-serif", color: '#1c1917', background: loading ? '#fafaf9' : '#fff', outline: 'none', lineHeight: 1.5, maxHeight: 100, transition: 'border-color 0.15s, box-shadow 0.15s', overflowY: 'auto' }}
  ...
  if (mode === 'page') {
    return (
      <div style={{ maxWidth: 760, margin: '0 auto', height: 'calc(100vh - 60px)', display: 'flex', flexDirection: 'column', fontFamily: "'DM Sans',sans-serif" }}>
        <div style={{ padding: '24px 28px 16px', borderBottom: '1px solid #eeede9', display: 'flex', alignItems: 'center', gap: 14, background: '#fff', flexShrink: 0 }}>
          <div style={{ width: 42, height: 42, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(99,102,241,0.35)' }}>
            <Sparkles size={20} color="white" />
          </div>
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#1c1917', letterSpacing: '-0.03em', marginBottom: 2 }}>Fintrax AI</h2>
            <p style={{ fontSize: 12, color: '#a8a49d', fontWeight: 500 }}>{expenses.length} transactions loaded · chat history saved</p>
          </div>
  ...
  return (
    <>
      {open && (
        <div style={{ position: 'fixed', bottom: 88, right: 24, width: 380, height: 560, background: '#fff', borderRadius: 20, border: '1px solid #e7e5e0', boxShadow: '0 24px 64px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column', overflow: 'hidden', zIndex: 1000, animation: 'panelIn 0.25s cubic-bezier(0.16,1,0.3,1)', fontFamily: "'DM Sans',sans-serif" }}>
          <div style={{ padding: '14px 16px', background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'white' }}>Fintrax AI</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', display: 'flex', alignItems: 'center', gap: 4 }}>
                <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#4ade80' }} />
                {expenses.length} transactions · history saved
              </div>
            </div>
            <div style={{ display: 'flex', gap: 4 }}>
              {[
                { icon: <RotateCcw size={12} />, onClick: clearHistory, title: 'Clear history' },
                { icon: <Maximize2 size={12} />, href: '/ai', title: 'Open full page' },
                { icon: <X size={14} />, onClick: () => setOpen(false), title: 'Close' },
              ].map((btn, i) => (
                btn.href
                  ? <a key={i} href={btn.href} title={btn.title} style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(255,255,255,0.15)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}
                  ...
                  : <button key={i} onClick={btn.onClick} title={btn.title} style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(255,255,255,0.15)', border: 'none', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <button onClick={() => setOpen(v => !v)} style={{ position: 'fixed', bottom: 24, right: 24, width: 56, height: 56, borderRadius: '50%', background: open ? '#1c1917' : 'linear-gradient(135deg,#4f46e5,#7c3aed)', border: 'none', cursor: 'pointer', zIndex: 1001, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px rgba(99,102,241,0.45)', transition: 'all 0.25s cubic-bezier(0.16,1,0.3,1)' }}
        ...
        {!open && anomalies.length > 0 && (
          <div style={{ position: 'absolute', top: 4, right: 4, width: 12, height: 12, borderRadius: '50%', background: '#f59e0b', border: '2px solid white' }} />
        )}
      </button>
